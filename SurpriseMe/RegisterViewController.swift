...
//                Toast.show(message: error!.localizedDescription, controller: self)
            ...
            let newUser = User.init(id: user.uid, email: user.email!, image: UIImage(named: "icons8-user"), firstName: self.firstName.text!, lastName: self.lastName.text!, dateOfBitrh: self.dateOfBirth.date, friends: [], myCart: [], sentFriendRequests: [], receivedFriendRequests: [],  myTreats: [], myOrders: [], getTreatsStatus: GetTreatStatus(rawValue: self.giftStatus!.selectedSegmentIndex)!, notifications: [], address: [:])
            let storage = Storage.storage().reference()
            storage.child(newUser.id).putData(newUser.image!.pngData()!, metadata: nil)
